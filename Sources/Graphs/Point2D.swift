...
public struct Point2D: Hashable, Codable, Sendable, ExpressibleByIntegerLiteral, ExpressibleByFloatLiteral {
    ...
    /// Add the x and y values of one point with a constant.
    ///
    /// - Parameter lhs: The first point.
    ///
    /// - Parameter rhs: The constant value to add to the x and y values of
    /// `lhs`.
    ///
    /// - Returns: A new Point2D resulting from adding `rhs` to `lhs.x` and
    /// `lhs.y`.
    public static func + (lhs: Point2D, rhs: Double) -> Point2D {
        Point2D(x: lhs.x + rhs, y: lhs.y + rhs)
    }

    /// Subtract the x and y values of one point with a constant.
    ///
    /// - Parameter lhs: The first point.
    ///
    /// - Parameter rhs: The constant value to subtract from the x and y values
    /// of `lhs`.
    ///
    /// - Returns: A new Point2D resulting from subtracting `rhs` from `lhs.x`
    /// and `lhs.y`.
    public static func - (lhs: Point2D, rhs: Double) -> Point2D {
        Point2D(x: lhs.x - rhs, y: lhs.y - rhs)
    }

    /// Multiply the x and y values of one point with a constant.
    ///
    /// - Parameter lhs: The first point.
    ///
    /// - Parameter rhs: The constant value to multiply the x and y values of
    /// `lhs` by.
    ///
    /// - Returns: A new Point2D resulting from multiplying `lhs.x` and `lhs.y`
    /// by `rhs`.
    public static func * (lhs: Point2D, rhs: Double) -> Point2D {
        Point2D(x: lhs.x * rhs, y: lhs.y * rhs)
    }

    /// Divide the x and y values of one point with a constant.
    ///
    /// - Parameter lhs: The first point.
    ///
    /// - Parameter rhs: The constant value to divide the x and y values of
    /// `lhs` by.
    ///
    /// - Returns: A new Point2D resulting from dividing `lhs.x` and `lhs.y` by
    /// `rhs`.
    public static func / (lhs: Point2D, rhs: Double) -> Point2D {
        Point2D(x: lhs.x / rhs, y: lhs.y / rhs)
    }
